...
## and Service.
#gensurf (fis, [1 2], 1); # this is unbelievably slow

# Calculate the Tip and Check + Tip using
# (Food-Quality, Service) = (4, 6).
[output, rule_input, rule_output, fuzzy_output] = ...
  evalfis ([-0.4 0], fis, 101);

# semicolon suppresses output :D
mymamdani = zeros(3, 5);
for berr = 1 : 11
    for bderr = 1 : 5
        terr = (berr - 6) * 0.2;
        ...
        [loutput, lrule_input, lrule_output, lfuzzy_output] = ...
          evalfis ([terr tderr], fis, 101);

        mymamdani(berr, bderr) = loutput(1);
    endfor
endfor
...
colors = ['r' 'b' 'm' 'g' 'r' 'b' 'm' 'g' 'r' 'b' 'm' 'g' 'r' 'b' 'm' 'g' 'r' 'b' 'm' 'g' ];
figure ('NumberTitle', 'off', 'Name', ...
        'Output of Fuzzy Rules 1-4 for Input = (E, dE)');
